...
- **`cargo test --all-targets` does not reach `fuzz` or `crosscheck`**, because exclusion is what makes the MSRV floor work. Both consume workspace crates' public APIs, so changing a public type — adding an enum variant field, renaming a method — compiles and tests clean locally and then fails CI in `fuzz`, `fuzz-run` or `crosscheck`. After any change to a public API, build them explicitly: `cargo check` in `implementation/rust/fuzz` and `cargo test` in `implementation/rust/crosscheck`. This has been hit more than once.
