...
\hypersetup{
  ...
  pdfsubject={Current Trahens protocol draft with U1, E1, R1, M2, W2, and T1 profiles}
}
...
Every \Tone{} transmission has the same complete length independently of frame class, logical message type, fragment position, protected-body length, retry status, or chaff status.
\end{definition}
...
During a pre-existing schedule epoch with origin $T_0$, slot interval $\Delta$, finite end time, and no declared overload transition, one cell is emitted in each directed slot independently of whether the selected encrypted frame is \textsc{Data}, \textsc{Ack}, or \textsc{Chaff}.
\end{definition}
...
\midrule
Fixed cell length & Specified and tested & Every \Tone{} record is 1,052 bytes. \\
...
$\Delta$ & schedule & Interval between two directed fixed-schedule slots. \\
...
\Tone{} supplies bounded adjacent-link selective recovery and two scheduler modes, but it is not a congestion controller. Recovery can amplify load on a saturated link; a fixed epoch can waste substantial bandwidth; and changing a schedule in response to queue pressure can itself disclose activity. The profile does not yet define rate negotiation, overload admission, fair sharing between route setup and data traffic, burst-loss recovery, forward-error correction, or end-to-end deadline selection. Fixed scheduling hides only the class of a cell inside an already active, non-overloaded link epoch. Timing and count correlation across links remain outside the claim.
...
\begin{enumerate}
  \item independent \Mtwo{}, \Wtwo{}, and \Tone{} codec implementations with differential tests and fuzzing;
  ...
  \item a congestion, overload, and schedule-negotiation profile with burst-loss and cross-link correlation experiments;
...
The semantic, fragmentation, and transport layers are distinct. Logical messages remain canonical and variable length; canonical fragments are bounded and reassembled before route semantics execute; and every \Tone{} adjacent-link cell has fixed length. Cumulative selective acknowledgements and bounded retries repair missing fragments without copying a recovery identifier across a relay. Optional fixed scheduling replaces idle slots with chaff and gives a narrowly defined public-trace equality property during a pre-existing non-overloaded epoch.

Route, reassembly, sender, acknowledgement, retry, schedule, registration, and redemption state have explicit lifetimes and budgets. The measured reliability improvement is accompanied by explicit retry and chaff costs. The security claims distinguish per-hop representation replacement and link-local schedule shape from global traffic-flow anonymity, and expose the directory/gateway and congestion-management trust boundaries.
...
An implementation claiming \Uone{}, \Eone{}, \Rone{}, \Mtwo{}, \Wtwo{}, and \Tone{} conformance MUST:
...
\begin{enumerate}
  ...
  \item support encrypted \textsc{Data}, \textsc{Ack}, and \textsc{Chaff} frame classes with a link-local non-zero transmission identifier;
  ...
  \item in fixed-schedule mode, emit exactly one equal-length cell per declared directional slot and specify finite epoch, rate, overload, and queue behavior;
  ...
  \item refrain from private-directory, directory/gateway-collusion, global traffic-flow, congestion-safety, post-quantum, or production-security claims unless separately specified and reviewed.
\end{enumerate}
...
\begin{table}[ht]
\centering
\caption{T1 adjacent-link frame classes}
\label{tab:t1frames}
...
\textsc{Chaff} & No & Occupy a declared fixed-schedule slot without route-semantic content. \\
\bottomrule
\end{tabularx}
\end{table}
