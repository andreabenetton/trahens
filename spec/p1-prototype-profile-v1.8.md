...
# Trahens v1.8 P1 prototype profile
...
non-normative future architecture. B1 work is not part of the v1.8 acceptance
gate.

### Restart safety is now structural

v1.7 and earlier took the link base key and epoch from configuration, so W2's
guarantees held only while an operator kept `(base key, epoch)` unique across
process lifetimes, and nothing detected a violation. That precondition is gone.
B1.1 derives both directional keys and the epoch from a handshake whose
...
key-reuse hazard; both are ordinary key management.
...
- separately started processes interoperate using the active v1.8 specification;
