...
## Active P1 profile: v1.7

v1.7 is the profile the implementation speaks. It rebuilds the end-to-end route
channel on a directional key schedule with counter nonces and a per-direction
replay window, and signs the gateway offer over a transcript binding the
protocol version, suite, reply key, and parameter digest. The protocol version
byte becomes `2`, so v1.6 and v1.7 do not interoperate
(`../docs/adr/0041-directional-route-channel.md`).

- [`core-v1.7.md`](core-v1.7.md) — mandatory interoperability semantics and evidence boundary.
- [`protocol-registry-v1.7.json`](protocol-registry-v1.7.json) — normative IDs, widths, limits, byte order, domains, and protection classes; current registry version 1.7.0.
- [`protocol-registry-v1.7.md`](protocol-registry-v1.7.md) — generated human-readable registry.
- [`p1-prototype-profile-v1.7.md`](p1-prototype-profile-v1.7.md) — executables, Linux harness, fuzzing, metrics, and acceptance gate.
- [`messages-v1.7.md`](messages-v1.7.md), [`state-machines-v1.7.md`](state-machines-v1.7.md), [`invariants-v1.7.md`](invariants-v1.7.md), and [`resource-accounting-v1.7.md`](resource-accounting-v1.7.md) — message roles, typed lifecycle, safety invariants, and concrete ceilings.
- `p1-conformance-vectors-v1.7.json` and `p1-conformance-corpus-v1.7.bin` — active canonical and noncanonical encoding corpus.

The mandatory v1.7 path is U1 + E1 + R1 + M2 + W2 + T1 + fixed T2/P1.
...
unless its own status says otherwise. A change here applies to v1.7; frozen
...
- [`network-bootstrap-b1.md`](network-bootstrap-b1.md) — future peer discovery, admission, authenticated adjacent-link establishment, gateway advertisement, and directory-root bootstrap. Core v1.7 still uses statically configured peers and base keys.
...
## Historical: v1.6 and earlier

**v1.6 and v1.5 are history.** Their files remain and still regenerate from
their own generators, so an implementation can be checked against either frozen
profile, but no current binary in this repository speaks them.
...
- `p1-conformance-vectors-v1.7.json` and `p1-conformance-corpus-v1.7.bin` — 32 independently encoded positive and negative M2 vectors for the active profile.
