...
# Trahens v1.6 P1 prototype profile
...
- a fanned-out branch commits the chain the initiator selected and releases the others.

The mandatory gate above is the fixed-T2, R1 profile. Two selectable
experimental profiles have their own gates and make their own, narrower claims:

- **adaptive T2** — negotiation occurs on a live link, steps are adjacent, and
  a stale or unanswered negotiation leaves the rate unchanged. It makes **no**
  fixed-trace claim, and the harness refuses to assert one when it is selected.
- **C1 eligibility** — a recipient serves a discovery addressed to it and
  declines one that is not, while relays rerandomise without deciding. It makes
  no endpoint-anonymity claim.

Neither may be cited as evidence for a mandatory gate line, and neither
replaces a mandatory arm.
