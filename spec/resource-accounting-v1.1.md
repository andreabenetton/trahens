...
# Trahens Core v1.1 resource accounting

- Status: Active research design
- Scope: U1, E1, R1, M2, W2 canonical fragmentation, and T1 reliability/scheduling
...
7. schedule slots and CHAFF bandwidth;
8. M2 parsing and route-protocol state;
9. rolling cryptographic-work and malformed-input windows;
10. node-global bytes, timers, registrations, and active routes.

Admission requires capacity in every applicable counter.
...
4. replay commitment;
5. T1 profile, frame class, and canonical field validation;
...
## 4. Mandatory T1 limits

A conforming deployment specifies finite values for:

- directed slot rate and schedule epoch duration;
...
Real DATA and ACK consume slots that would otherwise carry CHAFF; they do not increase `N` while the queue stays within capacity. The privacy cost is therefore explicit reserved bandwidth. A node MUST NOT promise a fixed schedule whose CHAFF reserve cannot be sustained.
...
- queue drops and malformed cells;
- duplicate fragments;
- peak and final sender/receiver transport state;
- public inter-arrival coefficient of variation;
- per-direction public trace cell count;
- cleanup completion.
