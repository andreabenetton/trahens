...
Core v0.9 specifies the first property, the protocol structure for the second, and the lifecycle necessary to test both. The third remains conditional on U1 and a declared mixing profile. For the fourth, C2 selects an anonymous rerandomizable RCCA security contract and integrates a symbolic ideal functionality. Core v0.9 additionally records an exact `k = 2` transcription and canonical byte audit of the concrete construction of Wang et al. [Wang2021](../docs/citation-audit.md#wang2021). The audit validates key generation, encryption, decryption, encoding, mutation rejection, and the linear strand equations. It also proves by counterexample that the source's literal finite-field map `u -> u mod q` is not multiplicative under ordinary `QR*_p` group multiplication. A concrete active-adversary claim therefore remains closed.
...
The reference simulator executes all five profiles together. Under C1 it performs the existing `ristretto255` URE transformations as a negative control. Under `c2-ideal` it performs replay-equivalent symbolic rerandomization and mutation rejection while retaining the concrete reply-key tweaks, nested candidate encryption, Ed25519 verification, COMMIT/READY proof checks, M2 encoding, W2 fragmentation and reassembly, fixed-size cell protection, and adjacent-link ChaCha20-Poly1305 authentication. The C2 symbolic backend is not deployable cryptography. The separate C2-K2 audit module transcribes the selected `k`-linear construction for equation and encoding review under reserved local suite `0x7f02`; it is fail-closed and cannot be selected by M2/W2.

## 4. Goals

Core v0.9 MUST provide:
...
12. explicit separation of the C2 active-security contract from the non-deployable symbolic backend and the fail-closed concrete transcription audit;
13. exact reporting of the C2-K2 finite-field non-homomorphism without treating partial arithmetic conformance as cryptographic approval or as a refutation of the generic framework.

## 5. Non-goals

Core v0.9 does not by itself provide:
...
- an approved concrete active-tagging-resistant C2 implementation or proof;
- Sybil resistance;
...
C2 uses suite identifier `0x0002` for the destination-eligibility capsule and the abstract `KeyGen`, `Enc`, `ReRand`, and `Dec` contract in `crypto-profile-c2.md`. The current protocol backend is symbolic and is not deployable cryptography. C1 (`0x0001`) remains a negative-control eligibility suite and supplies the currently executable reply-key, candidate-protection, signature, transcript, KDF, AEAD, and failure-handling components.

`crypto-profile-c2-k2.md` records a literal `k = 2` transcription of the concrete construction in Wang et al., CRYPTO 2021, Section 6.3 and Figure 6 [Wang2021](../docs/citation-audit.md#wang2021). The local audit encoding uses reserved suite `0x7f02`, contains 24 subgroup-checked group elements, and is 412 bytes. The reserved suite MUST NOT be accepted by M2 or W2. Its full rerandomization API MUST fail closed because the literal map `mu(u) = u mod q` is non-homomorphic under ordinary finite-field group multiplication; only an author-confirmed correction or independently reviewed replacement may reopen the gate.
...
Core v0.9 selects C2 as the active-security target for destination eligibility. The abstract capsule is produced by
...
Core v0.9 adopts the E1 profile in `event-lifecycle-profile-e1.md`. Every local state is valid on a half-open interval `[t_create, t_expire)`. Expiry is processed before a message assigned the same timestamp. Candidate delivery is processed before closure of a candidate window assigned the same timestamp.
...
C2 requires receiver-anonymous rerandomizable RCCA security and defines an explicit cross-hop tagging game. In the symbolic backend, arbitrary mutation is rejected by the first honest transformation and is not delivered to a separated downstream colluder. This validates protocol composition and failure handling, but it is not evidence for a concrete construction. The C2-K2 audit validates only a subset of the selected construction's arithmetic and encoding surface; its literal finite-field tag map is non-homomorphic, and its public rerandomization API is disabled. Core v0.9 MUST NOT claim concrete active-adversary unlinkability until the implementation gate in `crypto-profile-c2.md` and the audit gate in `crypto-profile-c2-k2.md` are closed.

## 26. Status of the design

Core v0.9 has executable M2 and W2 codecs and integrates a symbolic C2 eligibility functionality with the concrete C1 reply/signature components, cell fragmentation, bounded reassembly, and the E1 lifecycle. It also provides a deterministic C2-K2 equation and byte-encoding audit that is expressly non-networked and fail-closed. Passive structural claims remain conditional on U1, the selected cryptographic suites, W2, and an honest scheduling and mixing boundary. The C1 counterexample is reproducible; the C2 protocol contract and game harness are executable; the exact concrete construction has been partially transcribed but full rerandomization and proof mapping remain open. The protocol is a research design and is not suitable for production deployment.
