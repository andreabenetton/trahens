...
- encode every first-hop DISCOVER as a canonical variable-length M1 message;
- fragment each M1 message into one or more fixed-size W2 cells;
...
- accept a candidate only after W2 reassembly, canonical M1 decoding, and C1 verification succeed;
...
7. reserve the declared total M1 message length;
8. create a bounded context and transition to `COLLECTING`.

No branch, candidate, tentative, pending, or active route state exists at this point.

### COLLECTING

The context stores authenticated fragment bytes, a received-fragment bitmap, the declared total M1 length, and an expiry.
...
- decode exactly one canonical M1 message;
- reject trailing bytes, nonminimal varints, unknown mandatory profiles, malformed fields, or inconsistent message lengths.

On success, transition to `PARSED`; otherwise transition to `INVALID`.

### PARSED

The complete M1 message may enter its applicable protocol state machine. Admission, deadline, resource, and C1 checks still apply. A parsed message does not by itself authorize state creation.
...
1. require successful W2 authentication and complete M1 reassembly;
2. enforce the ingress-peer token bucket;
3. enforce per-node and node-global branch capacity;
4. validate M1 lifetime and propagation classes;
5. validate C1 points, rerandomize the URE capsule, and tweak the reply key;
...
- encode each child message canonically with M1 and assign a fresh W2 local message identifier per adjacent link;
...
- encode the resulting variable-length CANDIDATE as one canonical M1 message;
...
- consume one token before expensive fresh-branch processing, after W2/M1 validation;
...
- `MESSAGE_PARSED`: one complete canonical M1 message has been reconstructed and decoded.
...
A transition cannot skip an earlier state. Link authentication, W2 parsing, reassembly, or M1 decoding failure MUST NOT create branch, candidate, tentative, pending, or active route state.
