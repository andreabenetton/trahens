# Trahens Core v0.5 abstract messages

- Status: Research design draft
- Applies to: Core v0.5 with U1 and E1
- Encoding status: C1 cryptographic fields are concrete; the outer complete packet codec remains a future milestone

## 1. General requirements

Every control message is carried inside one adjacent-link record. C1 cryptographic fields use the canonical encodings in `crypto-profile-c1.md`; non-cryptographic packet framing remains abstract. The record header exposes only values required by the underlay. Protocol message type, fields, and padding are encrypted on the adjacent link.
...
The responder candidate payload is inside the innermost C1 reply capsule. Each C1 capsule starts with a 32-byte `TR-KEM-R255` encapsulated element and ends with a full 16-byte ChaCha20-Poly1305 tag. Relays MUST NOT receive the authenticated responder identity or service offer in plaintext.
