# Trahens Core v1.0 messages, M2 encoding, and W2 transport

- Status: Active research design
- Applies to: Core v1.0 with U1, E1, R1, M2, and W2; C1/C2 remain research controls and C1 remains the reply/signature component set
- Encoding status: M2 and W2 are concrete; C1 components are executable; R1 generic-gateway discovery is active; C1/C2 eligibility providers are research-only
...
15. perform suite-selected gateway eligibility, KEM, credential, AEAD, or signature work;
...
- `u1_profile_id`;
- `e1_profile_id`;
- `suite_id` = `0x0101` for active R1; `0x0001` and `0x0002` are research controls; `0x7f02` is rejected;
- `m2_profile_id` = `0x02`;
- zero reserved byte;
...
Propagate one independently transformed branch toward potential rendezvous gateways.
...
- suite-selected discovery field. For active R1 this is `service_query_nonce`, exactly 32 non-zero bytes.

For active R1 the field is a non-semantic 32-byte nonce. It identifies no endpoint and every honest relay replaces it independently for each child. Research-only C1 uses a 128-byte URE ciphertext and symbolic C2 uses an opaque 640-byte value. These research encodings are retained for attack regression and MUST NOT be enabled as active network suites.
...
- unchanged R1 service-query nonce from the previous hop.

### 4.4 Relay transformation

For every selected child, the relay generates a fresh branch token, blinds the reply public key, replaces the R1 service-query nonce, advances the bounded propagation control, constructs a new M2 message, assigns a new W2 message-local identifier, fragments canonically, generates new cell padding, and applies fresh link protection.

## 5. CANDIDATE

### 5.1 Purpose

Return an authenticated gateway offer through the reverse branch context while installing tentative hop-local route mappings.
...
The gateway payload is inside the innermost C1 reply capsule. It contains the short-lived gateway pseudonym, offer deadline, final reply public key, commit challenge, and signature. Relays do not receive the pseudonym or offer in plaintext.
...
READY confirms gateway route activation and completes reverse mappings. It uses the common route-control body with an incoming reverse label and a protected ready proof.
...
Present the endpoint-specific R1 capability only after the selected route is ACTIVE. RENDEZVOUS_OPEN is an end-to-end protected route-control operation; ordinary relays process only local active-route labels and never receive the capability plaintext.

### 8.2 Protected fields

- 32-byte non-zero capability;
- client nonce;
- descriptor expiration;
- endpoint-authentication or handshake material;
- bounded extension length.

The gateway hashes the capability under the R1 domain, looks up a live local commitment, validates the half-open deadline and gateway binding, atomically deletes the record, and only then releases the registered endpoint handle to its local rendezvous process. Replay, expiry, wrong gateway, malformed input, and local policy rejection map to one generic failure class.

The raw capability MUST NOT appear in DISCOVER, CANDIDATE, COMMIT, READY, W2 fragmentation metadata, ordinary relay logs, or detailed error telemetry.
...
## 15. Local protocol idempotency keys
...
- `(active_route_generation, capability_commitment)` at the gateway for RENDEZVOUS_OPEN, with atomic single-use consumption.

None is forwarded unchanged to another link.

## 16. E1 lifetime and window rules
...
## 17. Cryptographic validation and error behavior

A receiver first binds the M2 suite to the immutable W2 reassembly suite. R1 rejects an all-zero or wrong-length service-query nonce and otherwise determines eligibility from the node's local rendezvous-gateway role. C1 and symbolic C2 retain their malformed-input checks only in research configurations. Candidate signature, reply KEM, AEAD, and transcript failures map to one generic invalid-crypto result. The detailed reason is never transmitted.
...
## 18. Active-tagging boundary

Under active R1, a literal modification of the service-query nonce has no endpoint semantics and is erased by the next honest replacement. The C1 URE consistency pair is a negative-control construction: a compromised relay can place a persistent ratio relation in a freshly authenticated M2/W2 representation and a separated colluder can recognize that relation after honest C1 rerandomization. C2 instead requires that arbitrary modification be rejected by the first honest transformation or become indistinguishable from an honest replay-equivalent rerandomization. The symbolic C2 backend enforces this behavior by construction; it does not establish security of a concrete implementation.

The protocol MUST NOT infer endpoint-specific active-security from R1 nonce replacement. R1 removes endpoint selection from discovery but shifts trust to capability distribution and gateways. C1 and C2 remain research controls. Tag observations, honest-relay rejection, endpoint rejection, W2 authentication failures, M2 parsing failures, and reassembly failures are measured separately.
