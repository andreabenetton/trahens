...
- Supersedes: Core v0.3 as the active design draft
...
Core v0.4 restores the unlinkability objective of the 2020 draft by removing attempt-wide wire identifiers and by requiring a complete cryptographic transformation at every forwarding hop. Each outgoing branch receives a fresh link-local token, a fresh blinded reply public key, a rerandomized eligibility capsule, fresh forwarding labels, and a fresh adjacent-link ciphertext.
...
Core v0.4 specifies the first property and the protocol mechanism required for the second. The second is conditional on the U1 privacy profile. The third requires an independently specified traffic-scheduling profile and remains outside Core.
...
Core v0.4 retains U1 branch-local transformation and adds the E1 event lifecycle profile. The principal changes are:

- explicit local event time and half-open state deadlines;
- deterministic equal-time precedence for expiry, cancellation, control, candidate, discovery, and window events;
- initiator-local candidate windows and expanding-ring transitions;
- acceptance of delayed candidates from an earlier ring until a route decision is made;
- normative cancellation-race behavior;
- relay tentative state created during CANDIDATE return;
- forward COMMIT reservation and reverse READY activation;
- ready-gated exposure of a route to the data plane;
- deterministic cleanup after loss, duplication, reordering, expiry, or missing final messages;
- malicious fresh-branch generation and per-ingress-peer token-bucket admission.

The U1 cryptographic dependencies and limitations are unchanged. E1 is a lifecycle-correctness profile, not a new proof of traffic-flow unlinkability.

## 4. Goals

Core v0.4 MUST provide:
...
Core v0.4 does not by itself provide:
...
A relay receiving `DISCOVER` MUST apply the validation order in `messages-v0.4.md`. If admitted, it creates one branch context. It MUST NOT search for a network-wide duplicate identifier because none exists.
...
## 16. Event time and candidate windows

Core v0.4 adopts the E1 profile in `event-lifecycle-profile-e1.md`. Every local state is valid on a half-open interval `[t_create, t_expire)`. Expiry is processed before a message assigned the same timestamp. Candidate delivery is processed before closure of a candidate window assigned the same timestamp.
...
## 17. Tentative state, COMMIT, and READY
...
## 18. Cancellation, loss, and fresh-branch attacks
...
## 19. Exact replay and loop handling
...
## 21. Conditional unlinkability statement
...
## 23. Status of the design

Core v0.4 restores the protocol structure required for the legacy bit-pattern unlinkability objective. It does not yet provide a production cryptographic suite or a complete proof. `unlinkability-profile-u1.md` and `crypto-transcript-v0.1.md` define the remaining proof and implementation obligations.
