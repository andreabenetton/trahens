...
# Trahens Core v1.0 state machines

- Applies to: U1 branch-local transformation, E1 lifecycle, active R1 rendezvous discovery, C1 reply/signature components, M2 logical messages, and W2 cells
...
- obtain and validate the private R1 descriptor;
- allocate cumulative time, logical-message, cell-transmission, candidate, state, reassembly, and cryptographic-work budgets;
- transition to `PREPARING`.

### PREPARING

- generate independent first-hop C1 reply keys and fresh R1 service-query nonces;
...
- deduplicate gateway offers only inside local initiator state;
...
- on timeout, missing tentative state, capacity failure, gateway rejection, invalid reassembly, or invalid READY, transition to `CLOSING`.
...
The complete M2 message may enter its applicable protocol state machine. Admission, deadline, resource, suite-selected gateway eligibility, and C1 reply/signature checks still apply. A parsed message does not by itself authorize state creation.
...
5. validate the selected suite, replace the R1 service-query nonce, and tweak the C1 reply key;
...
- for R1, verify only canonical nonce syntax and the local rendezvous-gateway role; endpoint-specific capability redemption occurs after READY;
- enforce gateway offer and cryptographic-work budgets;
- if eligible, create one authenticated candidate payload and finite offer deadline;
- transition the offer to `OFFERED`.

### OFFERED

- build the exact 256-byte authenticated gateway payload and seal it with C1;
...
- reserve gateway route state for a finite interval;
...
- accept RENDEZVOUS_OPEN under active-route limits; on successful atomic capability redemption, start the local endpoint rendezvous process;
...
## 6. R1 capability registration and redemption

Gateway capability records use:

`ABSENT -> REGISTERED -> REDEEMING -> CONSUMED | EXPIRED`

### REGISTERED

- store only the domain-separated capability commitment, finite deadline, short-lived gateway pseudonym, and local endpoint handle;
- reject duplicate live registration for the same commitment;
- never expose the raw capability in discovery or candidate state.

### REDEEMING

Entered only from an ACTIVE route after an end-to-end protected RENDEZVOUS_OPEN is decoded at the gateway. Lookup and transition are atomic. The gateway verifies commitment, route binding, descriptor deadline, client handshake, and local policy. Any failure produces one generic result and releases the lock.

### CONSUMED

- the record is deleted before the endpoint handle is returned to the local rendezvous process;
- every replay fails;
- no transition returns to REGISTERED.

### EXPIRED

- remove the record and endpoint handle locally;
- a delayed RENDEZVOUS_OPEN cannot recreate it.

## 7. Ingress-peer token bucket
...
- `PROCESSED`: the applicable suite-selected gateway eligibility operation and any required C1 reply/signature operation succeed, and the state-machine transition commits.
...
## 10. Integrated candidate opening
...
The final canonical gateway payload is accepted only after its short-lived pseudonym is present in the private R1 descriptor and its final reply public key, offer deadline, challenge, descriptor-bound verification key, and Ed25519 signature all validate. Any failure transitions the candidate to `INVALID_CRYPTO` and leaves route selection unchanged.
...
Under active R1, a marked service-query nonce is replaced by the next honest relay and does not affect gateway eligibility. Under the C1 negative control, a compromised relay can replace the consistency pair with a chosen ratio relation and emit syntactically valid outgoing M2/W2 traffic. A separated colluder can test the relation after honest C1 rerandomization, and the endpoint later rejects the altered capsule.

Under the research-only symbolic C2 backend, arbitrary mutation is not a valid replay-equivalent rerandomization. The first honest C2 transformation rejects it, transitions the branch to the ordinary invalid-crypto path, and emits no transformed capsule to the downstream colluder. This validates the lifecycle placement of the C2-TAG game but does not establish security of a concrete C2 construction.
