...
- Date: 2026-07-30
- Scope: bounded privacy-enabled route discovery with branch-local contexts, fixed-size records, and an integrated cryptographic lifecycle
- Supersedes: Core v0.5
...
1. **record-length equality** - W1 gives every control record one 1,052-byte adjacent-link encoding;
...
- **W1**, one fixed-size authenticated adjacent-link record.

The reference simulator executes all four profiles together. It performs actual `ristretto255` URE transformations, reply-key tweaks, nested candidate encryption, Ed25519 verification, COMMIT/READY proof checks, fixed-size encoding, and adjacent-link ChaCha20-Poly1305 authentication.

## 4. Goals

Core v0.6 MUST provide:
...
4. one fixed adjacent-link record size for all control message classes;
...
Core v0.6 does not by itself provide:
...
W1 fixes the authenticated plaintext at 1,024 bytes and the complete reference link record at 1,052 bytes. Message type, profile identifiers, suite identifiers, logical fields, and padding are encrypted. The public 12-byte header contains only the link epoch and sequence.

A separate scheduling profile is required to define batching, release timing, cover traffic, and the minimum anonymity set. W1 alone does not hide when or in which direction a record crosses an edge.
...
W1 uses the exact layout in `wire-codec-w1.md`. A conforming implementation MUST reject non-canonical lengths, profiles, fields, points, scalars, and nested candidate lengths before state activation.
...
A relay receiving `DISCOVER` MUST apply the validation order in `messages-v0.6.md`. If admitted, it creates one branch context. It MUST NOT search for a network-wide duplicate identifier because none exists.
...
Core v0.6 adopts the E1 profile in `event-lifecycle-profile-e1.md`. Every local state is valid on a half-open interval `[t_create, t_expire)`. Expiry is processed before a message assigned the same timestamp. Candidate delivery is processed before closure of a candidate window assigned the same timestamp.
...
Therefore Core v0.6 MUST NOT claim active-adversary unlinkability. The exact attack, executable demonstration, and security gate are defined in `active-tagging-analysis.md`.

## 25. Status of the design

Core v0.6 has an executable fixed-size codec and integrates its concrete cryptography with the event-driven lifecycle. Passive structural claims remain conditional on U1, C1, W1, and an honest mixing boundary. The active-security gate is closed because the ratio-tag attack is reproducible. The protocol is a research design and is not suitable for production deployment.
