...
- Status: Evidence map for the acceptance gate in `spec/p1-prototype-profile-v1.8.md`
- Registry: 1.8.0
- Historical registries: v1.7, v1.6 and v1.5 retained only for reproducibility
...
| all canonical vectors pass and noncanonical encodings fail | `reference-and-spec`: `check_repo.sh` regenerates and byte-compares the 32-vector v1.8 corpus; the Rust `conformance` crate consumes it; Rust also consumes T1, T2, R1, and C1 component vectors |
...
The active v1.8 `DISCOVER` carries, unchanged from v1.6:
...
## What retaining v1.7, v1.6 and v1.5 means

The binaries implement **v1.8 only**. The protocol version byte is now `3`, so
v1.7 and v1.8 do not interoperate, and a v1.7 node cannot bring a link up at all
because it has no handshake to offer. v1.7 had already broken with v1.6 when the
route channel became directional; v1.6 had broken with v1.5 when `DISCOVER`
gained the separate 32-byte routing nonce.

The v1.7, v1.6 and v1.5 registries, vectors, corpora, and generated Markdown remain in
...
4. **Autonomous network bootstrap.** Link authentication is no longer part of
   this gap: B1.1 landed in v1.8, so adjacent peers authenticate each other and
