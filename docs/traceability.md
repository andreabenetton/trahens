...
| Implicit route activation | Tentative CANDIDATE, outward COMMIT, inward READY | `spec/state-machines-v1.3.md` |
| Directional labels | Peer-, direction-, epoch-, generation-, and deadline-bound random capabilities | `spec/invariants-v1.3.md` |
...
| Best-effort cleanup only | Independent local expiry plus advisory CANCEL/ABORT/CLOSE | E1 and v1.3 state machines |
| Unbounded convergence state | Explicit per-peer/global branch, reassembly, queue, and timer budgets | `spec/resource-accounting-v1.3.md` |
| Fragment loss | Hop-local cumulative selective ACK and bounded missing-fragment recovery | `spec/transport-profile-t1.md` |
| Retry equality | Fresh sequence, padding, authentication tag, and ciphertext per emission | T1 and v1.3 invariants |
