.PHONY: test crypto-vectors r1-vectors route-vectors cookie-vectors invitation-vectors advertisement-vectors seed-vectors t1-vectors t2-vectors t3-vectors t4-vectors c2-symbolic-vectors c2-k2-audit c2-k2-exhaustive r1-compare t1-compare t2-compare t3-compare t4-compare experiments sweep policy-compare unlinkability-compare lifecycle-compare tagging-compare fragmentation-compare c2-compare paper check reproduce clean
...
cookie-vectors:
	PYTHONPATH=simulator python tools/generate_cookie_vectors.py --output spec/b12-cookie-test-vectors.json
...
seed-vectors:
	PYTHONPATH=simulator python tools/generate_seed_vectors.py --output spec/b12-seed-manifest-vectors.json
