...
"""Trahens M1 variable-message codec and W2 fixed-cell framing.

M1 encodes protocol messages canonically without semantic padding. W2 fragments
an M1 message into one or more fixed-size adjacent-link cells, pads each cell
...
# M1 logical-message constants.
PROTOCOL_VERSION = 1
PRIVACY_PROFILE_U1 = 1
LIFECYCLE_PROFILE_E1 = 1
MESSAGE_PROFILE_M1 = 1
MESSAGE_COMMON_BYTES = 8
TOKEN_BYTES = 16
LABEL_BYTES = 16
MAX_LOGICAL_MESSAGE_BYTES = 16_384
CONTROL_PROTECTED_MAX = 8_192

# W2 cell constants. The public outer length remains the same as W1, but the
# encrypted body now carries a fragment header and a variable amount of M1 data.
CELL_BODY_BYTES = 1_024
CELL_HEADER_BYTES = 32
CELL_PAYLOAD_BYTES = CELL_BODY_BYTES - CELL_HEADER_BYTES
LINK_HEADER_BYTES = 12  # epoch: u32, sequence: u64
LINK_TAG_BYTES = 16
CELL_RECORD_BYTES = LINK_HEADER_BYTES + CELL_BODY_BYTES + LINK_TAG_BYTES
WIRE_PROFILE_W2 = 2
MAX_FRAGMENTS = ceil(MAX_LOGICAL_MESSAGE_BYTES / CELL_PAYLOAD_BYTES)
DEFAULT_REASSEMBLY_TIMEOUT_MS = 40
DEFAULT_REASSEMBLY_MAX_MESSAGES = 64
DEFAULT_REASSEMBLY_MAX_BYTES = 128 * 1024
    ...
    def accept(
        ...
        *,
        ...
        if entry is None:
            ...
            entry.fragment_count != cell.fragment_count
...
def _message_prefix(message_type: MessageType, body_length: int) -> bytes:
    return (
        bytes(
            [
                int(message_type),
        ...
        + C1_SUITE_ID
        + bytes([MESSAGE_PROFILE_M1, 0])
        + encode_varuint(body_length)
    )


def _encode_message(message_type: MessageType, body: bytes) -> bytes:
    encoded = _message_prefix(message_type, len(body)) + body
    if len(encoded) > MAX_LOGICAL_MESSAGE_BYTES:
        raise CodecError("logical message exceeds M1 maximum")
    return encoded


def _decode_message_envelope(encoded: bytes) -> tuple[MessageType, bytes]:
    ...
    if encoded[4:6] != C1_SUITE_ID:
        raise CodecError("unsupported cryptographic suite")
    if encoded[6] != MESSAGE_PROFILE_M1 or encoded[7] != 0:
    ...
    return message_type, encoded[cursor:]
...
def encode_discover(record: DiscoverRecord) -> bytes:
    ...
    return _encode_message(MessageType.DISCOVER, body)
...
def encode_candidate(record: CandidateRecord) -> bytes:
    ...
    return _encode_message(MessageType.CANDIDATE, body)
...
def encode_control(record: ControlRecord) -> bytes:
    ...
    return _encode_message(record.message_type, body)


def encode_chaff() -> bytes:
    return _encode_message(MessageType.CHAFF, b"")


def decode_message(
    encoded: bytes,
) -> DiscoverRecord | CandidateRecord | ControlRecord | MessageType:
    message_type, body = _decode_message_envelope(encoded)
    if message_type is MessageType.CHAFF:
        if body:
            raise CodecError("CHAFF body must be empty")
        return message_type
    if message_type is MessageType.DISCOVER:
        expected = TOKEN_BYTES + 4 + r255.POINT_BYTES + URE_BYTES
        if len(body) != expected:
            raise CodecError("invalid DISCOVER body length")
        cursor = 0
        ...
        try:
            ...
            capsule = URECiphertext.decode(body[cursor:])
        except (r255.RistrettoError, ValueError) as exc:
            raise CodecError("invalid DISCOVER cryptographic field") from exc
